...
/**
 * 이 관광지가 KTO 혼잡도 측정 대상인지.
 * 대상이 아니면 spots가 빈 배열로 온다 — 캘린더를 보여줄 게 없다는 뜻
 */
export function hasCongestionData(response?: GetCongestionResponse): boolean {
  return (response?.spots.length ?? 0) > 0;
}
 ...
 * 구간을 모를 때(응답 전이거나 측정 대상이 아닌 관광지)는 막지 않는다 — 잠깐 눌렸다가
 * 빈 달을 보는 편이 화살표가 깜빡이며 잠기는 것보다 덜 이상하다
 */
